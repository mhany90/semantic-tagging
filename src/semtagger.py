...
from keras.layers.convolutional import Convolution1D, Conv2D, MaxPooling1D, AveragePooling2D
...
def build_model():
    ...
    if args.chars:
        ...
        if args.bypass:
            ...
            #l = GRU(units=int(args.rnn_dim), return_sequences=True, dropout=0.1, recurrent_dropout=0.1, input_shape=(args.max_sent_len, word_embedding_dim), activation='relu', consume_less='cpu')(bypassed_char_rep)
            #l = GRU(units=int(args.rnn_dim/2), return_sequences=True, dropout=0.1, recurrent_dropout=0.1, activation='relu', consume_less='cpu')(l)

            #r = GRU(units=int(args.rnn_dim), return_sequences=True, go_backwards=True, dropout=0.1, recurrent_dropout=0.1, input_shape=(args.max_sent_len, word_embedding_dim), activation='relu', consume_less='cpu')(bypassed_char_rep)
            #r = GRU(units=int(args.rnn_dim/2), return_sequences=True, go_backwards=True, dropout=0.1, recurrent_dropout=0.1, activation='relu', consume_less='cpu')(r)
        ...
        if args.inception:
            for inc_idx in range(args.inception):
                ...
                one_x_one = Conv2D(args.max_sent_len, 1, 1, activation='relu', padding='same')(x)
                one_x_one = MaxPooling2D(pool_size=(2, 2), padding='same')(one_x_one)

                three_x_three = Conv2D(int(args.max_sent_len/4), 1, 1, activation='relu', padding='same')(x)
                three_x_three = Conv2D(args.max_sent_len, 3, 3, activation='relu', padding='same')(three_x_three)
                three_x_three = MaxPooling2D(pool_size=(2, 2), padding='same')(three_x_three)

                five_x_five = Conv2D(int(args.max_sent_len/4), 1, 1, activation='relu', padding='same')(x)
                five_x_five = Conv2D(args.max_sent_len, 5, 5, activation='relu', padding='same')(five_x_five)
                five_x_five = MaxPooling2D(pool_size=(2, 2), padding='same')(five_x_five)

                max_pool = MaxPooling2D(pool_size=(2, 2), padding='same')(x)
                max_pool = Conv2D(args.max_sent_len, 1, 1, activation='relu', padding='same')(max_pool) #TODO: Dim red.
            ...
            x = Conv2D(args.max_sent_len,kernel_size=(4, 8), activation='relu', padding='same', data_format ='channels_first')(x)
            print(x.shape)
            x = MaxPooling2D(pool_size=(2, 2), padding='same', data_format ='channels_first')(x)
            print(x.shape)
            x = Conv2D(args.max_sent_len,kernel_size=(4, 4), activation='relu', padding='same', data_format ='channels_first')(x)
            print(x.shape)      
            x = MaxPooling2D(pool_size=(2, 2), padding='same', data_format ='channels_first')(x)
            #avg_p = AveragePooling2D(pool_size=(maxlen_word, 2), padding='same')(x)
            print(x.shape)
            #channels = merge([max_p, avg_p], mode='concat')

            feature_size = int(args.max_word_len / 4) * int(args.char_embedding_dim / 4)
            print(args.max_sent_len, feature_size, x.shape)
            char_embedding = Reshape((args.max_sent_len, feature_size))(x)
    ...
    if args.rnn:
        # Bidirectional GRU
        l = GRU(units=int(args.rnn_dim), return_sequences=True, dropout=0.1, recurrent_dropout=0.1, input_shape=(args.max_sent_len, word_embedding_dim), activation='relu')(embedding)
        l = GRU(units=int(args.rnn_dim/2), return_sequences=True, dropout=0.1, recurrent_dropout=0.1, activation='relu')(l)

        r = GRU(units=int(args.rnn_dim), return_sequences=True, go_backwards=True, dropout=0.1, recurrent_dropout=0.1, input_shape=(args.max_sent_len, word_embedding_dim), activation='relu')(embedding)
        r = GRU(units=int(args.rnn_dim/2), return_sequences=True, go_backwards=True, dropout=0.1, recurrent_dropout=0.1, activation='relu')(r)
